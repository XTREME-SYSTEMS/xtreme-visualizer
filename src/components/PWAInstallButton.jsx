...
function buildStyle(cfg) {
  ...
  if (cfg.colorMode === "accent") {
    ...
    if (cfg.style === "solid" || cfg.style === "pill") {
      return { ...base, background: "var(--vx-accent)", color: "#0A0A0A", border: "1px solid var(--vx-accent)", boxShadow: "0 0 24px rgba(253,184,19,.28)" };
    }
    return { ...base, background: "linear-gradient(135deg, var(--vx-accent), var(--vx-accent-2))", color: "#0A0A0A", border: "1px solid var(--vx-accent)", boxShadow: "0 0 24px rgba(253,184,19,.28), 0 8px 24px rgba(0,0,0,.5)" };
  }
  if (cfg.style === "outline") {
