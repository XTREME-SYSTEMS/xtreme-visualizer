import { Home, Layers3, Menu, PackageSearch, ReceiptText, ScanLine } from 'lucide-react';
...
export function VisualXHeader({ title, eyebrow = 'VISUAL X', status = 'draft', action }: { title: string; eyebrow?: string; status?: 'ready' | 'partial' | 'blocked' | 'draft'; action?: ReactNode }) {
  return <header className="vx-header"><div className="vx-header__brand"><span className="vx-header__mark" aria-hidden="true">VX</span><div><small>{eyebrow}</small><h1>{title}</h1></div></div><div className="vx-header__actions"><VisualXStatusBadge status={status}>{status === 'draft' ? 'Component preview' : status}</VisualXStatusBadge>{action}</div></header>;
}
